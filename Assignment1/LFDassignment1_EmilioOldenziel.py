...
# NOTE: The comments are based on the provided program and are not edited after the changes
...
print(f"amount of samples {Y.size}")
print(Counter(Y))
...
for train_index, test_index in KFold(n_splits=K).split(X):
    ...
    for averaging in ["micro", "macro"]:
        f1 = f1_score(Y_test, Y_guess, average=averaging)
        f1_average[averaging].append(f1)
        print(f"f1 score using {averaging}: {f1}")
...
plt.show()
print(f"f1 average micro: {sum(f1_average['micro']) / float(len(f1_average['micro']))}") 
print(f"f1 average macro: {sum(f1_average['macro']) / float(len(f1_average['macro']))}")